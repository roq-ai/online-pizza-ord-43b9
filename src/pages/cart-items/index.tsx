...
type ColumnType = ColumnDef<CartItemInterface, unknown>;

interface CartItemListPageProps {
...
export function CartItemListPage(props: CartItemListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CartItemInterface>>(
    () => `/cart-items?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('cart', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('pizza', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'pizza',
          header: 'Pizza',
          accessorKey: 'pizza',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/pizzas/view/${record.pizza?.id}`}>
              {record.pizza?.name}
            </Link>
          ),
        }
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/cart-items/view/${record.id}`} passHref legacyBehavior>
                <Button
