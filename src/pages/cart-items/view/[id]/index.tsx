...
import { getCartItemById } from 'apiSdk/cart-items';
import { CartItemInterface } from 'interfaces/cart-item';

function CartItemViewPage() {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('cart_item', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/cart-items/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('pizza', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Pizza"
                    text={
                      <Link as={NextLink} href={`/pizzas/view/${data?.pizza?.id}`}>
                        {data?.pizza?.name}
                      </Link>
                    }
                  />
