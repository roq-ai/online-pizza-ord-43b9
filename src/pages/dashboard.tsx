...
import { getCartItems } from 'apiSdk/cart-items';
import { CartItemInterface } from 'interfaces/cart-item';
import { getOrders } from 'apiSdk/orders';
import { OrderInterface } from 'interfaces/order';

import { CartItemListPage as ShowCaseList } from 'pages/cart-items';
...
function DashboardPage() {
  ...
  const cartItemStat = useSWR<PaginatedInterface<CartItemInterface>>(
    () => `/cart-items`,
    () => getCartItems({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Cart Items',
      result: cartItemStat?.data?.totalCount,
      isLoading: cartItemStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Orders',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`cart-items`} passHref legacyBehavior>
                  <Button
