...
import { getPizzaById } from 'apiSdk/pizzas';
import { PizzaInterface } from 'interfaces/pizza';
import { CartItemListPage } from 'pages/cart-items';

function PizzaViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<PizzaInterface>(
    () => (id ? `/pizzas/${id}` : null),
    () =>
      getPizzaById(id, {
        relations: [],
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('pizza', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/pizzas/edit/${id}`} passHref legacyBehavior>
                    <Button
